...
const Timer = require('@liqd-js/timer');
...
module.exports = class WebsocketServer extends EventEmitter
{
	constructor( options )
	{
		...
		this._server.on( 'upgrade', async( request, socket ) =>
		{
			...
			{
				request.query = Querystring.parse( ~request.url.indexOf('?') ? request.url.substr( request.url.indexOf('?') + 1 ) : '', this._options?.query?.types );
